...
Use the high-level tracing/risk tools first. Fall back to `trace_call_path` for deeper graph-native traversal.

## Workflow

### Step 1: Get fast caller context

```
get_callers(symbol="ProcessOrder")
```

For file-level impact:

```
get_file_context(path="src/orders/service.go")
```

### Step 2: Discover the exact function name when needed
...
### Step 3: Trace callers (who calls this function?)
...
### Step 4: Trace callees (what does this function call?)

```
trace_call_path(function_name="ProcessOrder", direction="outbound", depth=3)
```

### Step 5: Full context (both callers and callees)
...
For changed files that you already know:

```
get_change_risks(paths=["src/orders/service.go", "src/orders/http.go"])
```

This returns affected symbols, related files, related tests, risk factors, and a compact blast-radius score.
