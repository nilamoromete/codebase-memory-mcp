...
High-quality parsing through [tree-sitter](https://tree-sitter.github.io/tree-sitter/) AST analysis across 158 languages — producing a persistent knowledge graph of functions, classes, call chains, HTTP routes, and cross-service links. 17 MCP tools. No hosted service or API key; this package requires Node.js to manage and launch the cached native runtime set. Plug and play across 43 automatic/conditional client surfaces.
...
- **17 MCP tools** — search, trace, architecture, impact analysis, evidence-backed edit planning and change-risk checks, index-coverage validation, Cypher queries, dead code detection, cross-service HTTP linking, ADR management, and more.
...
codebase-memory-mcp cli get_edit_plan '{"path": "src/main.c", "mode": "compact"}'
codebase-memory-mcp cli get_change_risks '{"diff_mode": "working_tree"}'
...
| **Workflow evidence** | `check_index_coverage`, `get_edit_plan`, `get_change_risks` |
| **Advanced** | `manage_adr`, `ingest_traces` |

Call `get_edit_plan` before editing the primary file. Before completion, call
`get_change_risks` for explicit edited paths or `diff_mode="working_tree"`,
then run the recommended tests. These workflow tools are read-only and report
freshness, coverage, confidence, and bounded evidence; they do not mutate the
repository or run tests for you.
